...
class _JoinRoomScreenState extends State<JoinRoomScreen> {
  ...
  Future<void> _joinRoom() async {
    ...
    if (code.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Masukkan kode room terlebih dahulu', style: GoogleFonts.poppins(fontSize: 12)),
    ...
    try {
      ...
      if (roomData == null) {
        setState(() { _isLoading = false; _errorMessage = 'Kode room "$code" tidak ditemukan. Cek kembali kodenya.'; });
        return;
      }
      ...
      final items = rawItems.map((raw) {
        ...
        return {
          ...
          'quantity': qty,
          'unitPrice': unitPrice,
          'totalPrice': unitPrice * qty, // hitung ulang sesuai qty asli
          'checked': false,
        };
      }).toList();
      setState(() { _storeName = roomData['storeName'] as String? ?? 'Belanja'; _items = items; _hasJoined = true; _isLoading = false; });
    } catch (e) {
      setState(() { _isLoading = false; _errorMessage = 'Gagal bergabung: ${e.toString()}'; });
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Gabung Room', style: GoogleFonts.poppins(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w600)),
      ),
  ...
  Widget _buildJoinCard() => Container(
    ...
    child: Column(children: [
      Text('Gabung Room Split', style: GoogleFonts.poppins(fontSize: 15, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
      const SizedBox(height: 4),
      Text('Masukkan kode dari temanmu', style: GoogleFonts.poppins(fontSize: 12, color: AppColors.textSecondary)),
      const SizedBox(height: 16),
      TextField(
        ...
        maxLength: 5,
        style: GoogleFonts.poppins(fontSize: 22, fontWeight: FontWeight.w700, letterSpacing: 8, color: AppColors.primary),
        decoration: InputDecoration(
          counterText: '',
          hintText: '_ _ _ _ _',
          hintStyle: GoogleFonts.poppins(fontSize: 22, letterSpacing: 8, color: AppColors.textHint),
        ),
      ),
      ...
      SizedBox(width: double.infinity, height: 48,
        child: ElevatedButton(
          ...
          child: _isLoading
              ...
              : Text('Gabung Sekarang', style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w600, color: Colors.white)),
        ),
      ),
    ]),
  );
  ...
  Widget _buildChecklistSection() => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text('Pilih Barangmu', style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
        Text(_storeName, style: GoogleFonts.poppins(fontSize: 11, color: AppColors.textSecondary)),
      ]),
      const SizedBox(height: 12),
        ...
        return Column(children: [
          ...
          Padding(
            ...
            child: Row(children: [
              // Checkbox
              GestureDetector(
                onTap: () => setState(() => _items[i]['checked'] = !checked),
                child: Container(
                  width: 22, height: 22,
              ...
              // Tombol qty
              Row(children: [
                _qtyBtn(Icons.remove, qty > 1 ? () => _updateQty(i, -1) : null),
              ...
              const SizedBox(width: 8),
              // Total harga item
              SizedBox(width: 68, child: Text(_formatRupiah(totalPrice), textAlign: TextAlign.end,
  ...
  Widget _buildMyTotal() => Container(
    ...
    child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      Text('Total tagihan kamu', style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.w500, color: AppColors.primary)),
      Text(_formatRupiah(_myTotal), style: GoogleFonts.poppins(fontSize: 20, fontWeight: FontWeight.w700, color: AppColors.primaryDark)),
    ]),
  );
  ...
  Widget _buildConfirmButton() => SizedBox(
    ...
    child: ElevatedButton.icon(
      onPressed: () {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Pembayaran dikonfirmasi! ✅', style: GoogleFonts.poppins(fontSize: 12)),
      ...
      label: Text('Konfirmasi & Tandai Bayar',
